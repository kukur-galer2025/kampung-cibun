...
export default function GalleriesAdmin() {
  ...
  const fetchData = async () => { try { setItems(await (await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://apikampungcibun.amania.id'}/api/galleries`)).json()); } catch {} finally { setLoading(false); } };
  useEffect(() => { fetchData(); }, []);
